...
This document provides detailed instructions for the `autoslurm-configuration` command.
...
The configuration file `~/.autoslurmconfig` stores the following information:
...
When executed for the first time, `autoslurm-configuration`
will create the configuration file '~/.autoslurmconfig' if it does not exist.
...
`autoslurm-configuration` command. It must always be present in the configuration
file.
...
└── data
```
`$AUTOSLURM` is an environment variable automatically appended to your `.bashrc` file
by `autoslurm-configuration` using the path specified in the `path` field.
...
SLURM shell automatically generated by `autoslurm` and output files created by SLURM.
...
- In case multiple jobs are created simultaneously with the same name,
    `autoslurm` will append a unique timestamp to each by adding seconds to the
    timestamp.
...
SLURM script generated by `autoslurm` have this generic structure:
...
then you can create multiple configurations in the `~/.autoslurmconfig` file
...
You can use this name also in the `~/.autoslurmconfig` file to specify the hostname
field.
